...
These scripts ease the environment setup procedure required to build AllScale.
...
A folder `third_party` should be located inside the AllScale build directory,
...
different versions of the same package. Example:
    ...
    drwxr-xr-x.  5 alex   dps 4.0K Nov 18 15:14 cmake-3.6.1/
        ...
    $ ls -l $ALLSCALE_SRC/build/third_party
...
Adjust your environment variables as needed, for example:
    ...
    export PATH="$ALLSCALE_SRC/third_party/ruby/bin:$PATH"
    export PATH="$ALLSCALE_SRC/third_party/valgrind/bin:$PATH"
