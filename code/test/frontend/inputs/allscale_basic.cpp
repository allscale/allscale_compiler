...
#define SIMPLE_PREC_IR R"(
	...
	def struct __any_string__step {
		const function IMP__operator_call_ = (v133 : ref<int<4>,f,f,plain>, v134 : ref<(recfun<ref<int<4>,t,f,cpp_ref>,int<4>>),f,f,plain>) -> treeture<int<4>,f> {
			recfun_to_fun(
...
#define SIMPLE_FUN_IR R"( var ref<(ref<int<4>,t,f,cpp_ref>) => treeture<int<4>,f>,f,f,plain> simpleFun = )"
