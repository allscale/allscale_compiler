...
int main() {
	...
	#pragma test expect_ir(R"(
		def IMP_SimplestDI :: const function IMP_getInvolvingCleanups = () -> real<8> {
			return *data_item_element_access(ref_kind_cast(this, type_lit(cpp_ref)), 0, type_lit(ref<real<8>,f,f,cpp_ref>));
		};
		{
			...
			instantiate(IMP_SimplestDI::IMP_getInvolvingCleanups, lit("PARSER_UNRESOLVED_IMP_SimplestDI::IMP_getInvolvingCleanups" : const IMP_SimplestDI::() -> real<8>))(v0);
		}
	)")
	{
		SimplestDI di;
