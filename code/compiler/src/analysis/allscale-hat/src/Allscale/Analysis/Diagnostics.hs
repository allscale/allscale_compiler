...
globalVariableDiagnosis addr = diagnosis diag addr
  where
    diag = Diagnosis GlobalVariableDiagnosis "global" globalVariableDiagnosis ops
...
uncertainAccessDiagnosis addr = diagnosis diag addr
  where
    diag = Diagnosis UncertainAccessDiagnosis "uncAcc" uncertainAccessDiagnosis ops
    ...
    dep _ _ = [Solver.toVar diReferenceVar]
