...
main = do
    ...
    libs  <- head . fst <$> globDir [(compile "install/**/libHSallscale-hat-*.so")] ".stack-work"
    ...
    isOod <- outOfDate latestts (args !! 0 </> "libHSallscale-hat.so")

    -- copy original + simple name
    if isOod
        then do
            putStrLn $ "Copying over lib " ++ show latestlib
            copyFile latestlib  (args !! 0 </> "libHSallscale-hat.so")
